...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (!title || !logo || rating === undefined || timing === undefined || !style || !website || !code || enableStars === undefined) {        return res.status(400).json({ message: 'All fields are required' });
    }

    // Validate rating
    const validRating = rating === 'yes' ? true : rating === 'no' ? false : rating;
    if (validRating === null) {
